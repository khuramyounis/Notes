...
suspend fun NoteDao.returnOrderedQuery(
    ...
    return when{
        filterAndOrder.contains(ORDER_BY_DESC_DATE_UPDATED) ->
            searchNotesOrderByDateDESC(query = query, page = page)

        filterAndOrder.contains(ORDER_BY_ASC_DATE_UPDATED) ->
            searchNotesOrderByDateASC(query = query, page = page)

        filterAndOrder.contains(ORDER_BY_DESC_TITLE) ->
            searchNotesOrderByTitleDESC(query = query, page = page)

        filterAndOrder.contains(ORDER_BY_ASC_TITLE) ->
            searchNotesOrderByTitleASC(query = query, page = page)

        else -> searchNotesOrderByDateDESC(query = query, page = page)
    }
}
